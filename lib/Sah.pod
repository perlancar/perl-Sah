...
Normally, a clause tests data. With this attribute, you can change the input to
something else. Possible values:
...
Without C<check.input> set to the C<len> property, C<$_> in the expression will
still refer to the data (the string).
...
Illustration:
 ...
 int + {"in": [1,2,3,4,5]}, {"[merge-]in": [4]}  // will be merged and become:
 int + {"in": [1,2,3,  5]}
