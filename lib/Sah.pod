...
additional clause (C<< min=>0 >>).

Overriding and removing base schema's clauses are also possible via a process
called merging. For example:

 # schema: pos_even_or_odd (positive even or odd numbers)
 [pos_even => {"[merge!]div_by"=>2}] # remove the div_by clause

In the above example, C<pos_even> removes its base schema's C<div_by> clause,
making C<pos_even_or_odd> effectively equivalent to positive integer.
...
A type can have B<clauses>, which mostly declare constraints. Clauses are like
functions, they are evaluated against data and return a value. For the clause to
succeed, the return value must be true. For the schema to succeed, all
constraint clauses must be true. Aside from declaring constraints, clauses can
also declare other stuff like default value (the C<default> clause), store
metadata (the C<summary>, C<description>, C<tags> clauses), etc.
