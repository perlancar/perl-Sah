...
Valid value: C<fatal>, C<error>, C<warn>. Normally, when clause checking fails,
an error is generated and it causes validation of the whole schema to fail. If
...
more attributes with language code suffixes. For example:
...
=item * ok_err_msg[.LANGCODE]

Just like C<err_msg>, but this error message will be used when clause succeeds
and there is C<.min_nok> or C<.max_ok> attributes defined. You can add
translations by adding more attributes with language code suffix. For example:

 ["str", {"!in": ["root", "admin"], // shortcut for "in": ... + "in.max_ok": 0
          "in.ok_err_msg":       "Sorry, username is reserved",
          "in.ok_err_msg.id_ID": "Maaf, nama user dilarang digunakan"
 }]
