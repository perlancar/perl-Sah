...
clause is allowed, unless the C<allow_extra_keys>

If C<keys> clause is specified and C<allow_extra_keys> clause is set to true,
any hash key is allowed.

If C<allowed_keys> and/or C<allowed_keys_re> clause is specified, then only keys
matching those clauses are allowed.
...
=head3 How to express mutual failure ("if A fails, B must also fails")?
