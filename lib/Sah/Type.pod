...
might then update its own implementation to match C<vocal>:
 ...
 ["vocal", {"base_v":2, "cset":{"match":"\\A[a-z]\\z"}, "match.op":"not"}]
...
Evaluate a clause named C<CNAME> with its value C<CVAL>. Example:

 ["int", {"clause": ["min": 0]}] // equivalent to ["int", {"min": 0}]

Putting other clauses inside the C<clause> clause is useful when combined with
the C<.op> attribute, example:

 ["int", "clause", [["div_by", 5], ["between", [1, 10]]], "clause.op", "or"]

The above schema requires data to be divisible by 5 I<or> between 1 and 10.
