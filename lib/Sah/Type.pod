...
=head3 (TODO: req_some_keys : [ min, max, ARRAY[STR] ])

=head3 dep_one : [ STR|ARRAY[STR], ARRAY[STR] ]
...
argument exists. Example:
 ...
 ["hash", "dep_one", ["postcode", ["address"]]]

The "postcode" key can only be specified if "address" key exists.

Another example:

 ["hash", "dep_one", ["input_format", ["input_value", "input_file"]]
...
Yet another example:
 ...
 ["hash", "dep_one", [["input_format", "input_is_yaml", "input_is_json"], ["input_value", "input_file"]]
...
In the above example, you can also use "dep_one" clause for the same effect
...
=head3 req_dep_one : [ STR|ARRAY[STR], ARRAY[STR] ]
